...
echo "$($PSQL "SELECT distinct name FROM teams where name in ((select name from games left join teams on games.winner_id=teams.team_id where round='Eighth-Final' and year=2014) union (select name from games left join teams on games.opponent_id=teams.team_id where round='Eighth-Final' and year=2014)) order by name asc")"
